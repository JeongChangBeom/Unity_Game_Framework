...
namespace GameFramework.SaveLoad
{
    ...
    internal static class JsonUtilityCodec
    {
        ...
        {
            Wrapper<T> wrapper = new Wrapper<T> { TypeName = typeof(T).FullName, Value = value };
        ...
        public static bool TryFromJson<T>(string json, out T value)
        {
            ...
            try
            {
                ...
                if (probe != null && !string.IsNullOrEmpty(probe.TypeName) && probe.TypeName != expectedTypeName)
                {
                    Debug.LogError($"[JsonUtilityCodec] 저장된 타입({probe.TypeName})과 요청한 타입({expectedTypeName})이 달라 로드를 거부합니다.");
                    return false;
                }
